...
const settings = {
  ...
  name: `polygoner-superposed-${numSides}-${Date.now()}`
};
...
const drawPolygon = (ctx, radius) => {
  ...
  for (let i = 0; i < numSides; i++) {
    ...
    x = Math.cos(currentAngle + angle * 0.5) * radius / (ratio * ratio * ratio);
    y = Math.sin(currentAngle + angle * 0.5) * radius / (ratio * ratio * ratio);
    ctx.lineTo(x, y);
